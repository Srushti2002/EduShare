...
const updatePlaylistProgress = async (req, res) => {
  try {
    ...
    if (!playlistId || typeof progress !== 'object') {
      return res
        .status(400)
        .json({ error: 'playlistId and progress object are required' });
    }
    ...
    if (user.role !== 'student') {
      return res
        .status(403)
        .json({ error: 'Only students can update playlist progress' });
    }
    ...
    for (const [videoId, percent] of Object.entries(progress)) {
      mergedProgress[videoId] = Math.max(
        typeof mergedProgress[videoId] === 'number'
          ? mergedProgress[videoId]
          : 0,
    ...
    if (typeof overallPlaylistProgress === 'number') {
      const prevOverall = user.overallPlaylistProgress.get(
        playlistId.toString()
      );
...
const calculateOverallProgress = async (req, res) => {
  try {
    ...
    const enrolledPlaylists = user.followingPlaylists.map((id) =>
      id.toString()
    );
